...
${TOOLCHAIN}ar d libavcodec/libavcodec.a inverse.o
${TOOLCHAIN}ld -rpath-link=$NDK/toolchains/huyu/my-android-toolchain-arm/sysroot/usr/lib \
-L$NDK/toolchains/huyu/my-android-toolchain-arm/sysroot/usr/lib \
-L$PREFIX/ffmpeg/lib \
-L$PREFIX/lib \
-soname libffmpeg.so -shared -nostdlib -z noexecstack -Bsymbolic --whole-archive --no-undefined -o $PREFIX/ffmpeg/lib/libffmpeg.so \
...
-lc -lm -lz -ldl -llog -lfdk-aac -lexpat -lfribidi -lfreetype -lfontconfig -lass -lpng16 -lx264 -ldl --dynamic-linker=/system/bin/linker $NDK/toolchains/huyu/my-android-toolchain-arm/lib/gcc/arm-linux-androideabi/4.9.x/libgcc.a
