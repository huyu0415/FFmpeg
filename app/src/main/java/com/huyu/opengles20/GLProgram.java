 ...
 * 1. new GLProgram()
 *
 * 2. buildProgram()    目的要生成一个program，作用是用来将YUV->RGB，
 *      其中用到了2个shader（shader就相当于一个小运算器，它运行一段代码），第1个shader运行VERTEX_SHADER里的代码，目的是将坐标作为参数传入第2个shader；第2个shader来做YUV->RGB的运算。
 *
 * 3. buildTextures()   要生成3个贴图，分别为了显示R/G/B数据，三个贴图重合在一起，显示出来的就是彩色的图片。
 *
 * 4. drawFrame()       使用program来做运算，并真正去做画这个动作
 *
 * 向Renderer提供计算单元
 * 负责YUV数据转换成RGB -> GL中的Program/Shader
 */
public class GLProgram {
    ...
    public void buildTextures(Buffer y, Buffer u, Buffer v, int width, int height) {
        ...
        if (videoSizeChanged) {
           ...
           Log.d("huyu","buildTextures videoSizeChanged: w=" + _video_width + " h=" + _video_height);
        }

        // building texture for Y data
        if (_ytid < 0 || videoSizeChanged) {
            if (_ytid >= 0) {
               Log.d("huyu","glDeleteTextures Y");
            ...
            _ytid = textures[0];
           Log.d("huyu","glGenTextures Y = " + _ytid);
        }
        ...
        if (_utid < 0 || videoSizeChanged) {
            ...
            checkGlError("glGenTextures");
            _utid = textures[0];
           Log.d("huyu","glGenTextures U = " + _utid);
        }
        ...
        if (_vtid < 0 || videoSizeChanged) {
            ...
            checkGlError("glGenTextures");
            _vtid = textures[0];
           Log.d("huyu","glGenTextures V = " + _vtid);
        }
    ...
    public int createProgram(String vertexSource, String fragmentSource) {
        ...
        // just check
       Log.d("huyu","vertexShader = " + vertexShader);
       Log.d("huyu","pixelShader = " + pixelShader);
        ...
        if (program != 0) {
            ...
            if (linkStatus[0] != GLES20.GL_TRUE) {
                Log.e("huyu","Could not link program: ", null);
                Log.e("huyu", GLES20.glGetProgramInfoLog(program), null);
    ...
    private int loadShader(int shaderType, String source) {
        ...
        if (shader != 0) {
            ...
            if (compiled[0] == 0) {
                Log.e("huyu","Could not compile shader " + shaderType + ":", null);
                Log.e("huyu", GLES20.glGetShaderInfoLog(shader), null);
    ...
    private void checkGlError(String op) {
        ...
        while ((error = GLES20.glGetError()) != GLES20.GL_NO_ERROR) {
            Log.e("huyu","***** " + op + ": glError " + error, null);
