 ...
 * @param width         相机宽
 * @param height        相机高
 * @param urlpath       直播流服务器地址  "rtmp://192.168.9.135:1935/wstv/home";
 * @return
 */
int cameraLivePlayerInit(int width, int height, const char *urlpath) {
    //const char *out_path = "rtmp://192.168.9.135:1935/wstv/home";
    yuv_width = width;
    yuv_height = height;
...
//    av_log_set_callback(custom_log);
    ...
    //output initialize
    avformat_alloc_output_context2(&ofmt_ctx, NULL, "flv", urlpath);
    ...
    if (avio_open(&ofmt_ctx->pb, urlpath, AVIO_FLAG_READ_WRITE) < 0) {
...
int cameraLivePlayerStart(signed char *yuv) {
    ...
    memcpy(pFrameYUV->data[0], yuv, y_length);
    for (i = 0; i < uv_length; i++) {
        *(pFrameYUV->data[2] + i) = *(yuv + y_length + i * 2);
        *(pFrameYUV->data[1] + i) = *(yuv + y_length + i * 2 + 1);
    }
